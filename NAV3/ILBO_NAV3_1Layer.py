...
class ActorBuffer(Buffer):
    ...
    def update(
        ...
        with tf.GradientTape() as tape:
            ...
            # Next State Mean for new Theta
            for i in range(decZoneCtrArr.shape[0]):
                dist = tf.norm(state_batch - decZoneCtrArr[i], axis = 1)
                deceleration = 2.0 / (1.0 + tf.math.exp(-decZoneDecayArr[i] * dist)) - 1.0
                if i == 0:
                    totalDec = deceleration[:, tf.newaxis]
                else:
                    totalDec *= deceleration[:, tf.newaxis]
            
            nextState_Mean = state_batch + totalDec * actions
                        
            ndim = state_batch.get_shape().as_list()[1]
            diffMatrix = next_state_batch - nextState_Mean
            covMatrix = tf.constant([[mov_var_vec[0], 0.], [0., mov_var_vec[1]]], dtype = tf.float32)
            
            prob_nextState = tf.math.exp(-0.5 * tf.reduce_sum(tf.matmul(diffMatrix, tf.linalg.inv(covMatrix)) * diffMatrix, 1)) / tf.math.sqrt((2 * math.pi)**ndim * tf.linalg.det(covMatrix))
            prob_nextState /= max_pdf # Divide by maximum possible pdf
            prob_nextState += 1e-12 # Small value tolerance
            ...
            actor_loss = -tf.math.reduce_mean(gamma * (V_nextState - V_currState) * tf.math.log(prob_nextState))
...
def policy(actor_model, state, noise_object, t):
    ...
    sampled_actions = sampled_actions.numpy() + (noise * (upper_bound - lower_bound))
...
def set_start_state(rddl_env, start_state):
    ...
    # Set Start State
    state['location/1'] = np.array(start_state)
    env._state['location/1'] = state['location/1']
...
def simulate_test(rddl_env, start_state, total_episodes, seed):
    ...
    for ep in range(total_episodes):
        ...
        if ep == 0:
            ...
            rddl_env, state, t = set_start_state(rddl_env, np.random.uniform([0] * num_states, [20] * num_states))
        done = False
        ...
        while not done:
            ...
            curr_state = state['location/1'].astype(np.float32)            
            
            tf_state = tf.expand_dims(tf.convert_to_tensor(curr_state), 0)
            
            action = OrderedDict({'move/1': policy(mm_actor, 
                                                   tf_state, 
            ...
            reward, nextState = reward.astype(np.float32), next_state['location/1'].astype(np.float32)
            
            q_buffer.record(curr_state, action['move/1'].astype(np.float32), reward, nextState, done)
            actor_buffer.record(curr_state, action['move/1'].astype(np.float32), reward, nextState, done)
...
def test_actor_loop(filename, rddl_env, start_state):
    ...
    for i in range(test_loops):
        ...
        while not done:
            ...
            action = OrderedDict({'move/1': policy(mm_actor, tf_state, lambda : np.array([0] * num_actions), t)})
            next_state, reward, done, info = rddl_env.step(action)
...
def plot_graphs(dirName, lQsaLoss, lActorLoss, lQsaGrad, lActorGrad, lTrajList):
    ...
    trajectory_df = lTrajList[-1][-1]
    
    x = trajectory_df['location(x)'].to_numpy()
    y = trajectory_df['location(y)'].to_numpy()
    
    plt.figure()
    plt.quiver(x[:-1], y[:-1], x[1:]-x[:-1], y[1:]-y[:-1], scale_units='xy', angles='xy', scale=1)
    plt.plot(1.5, 3.0, color='red', marker='x', linestyle='dashed', linewidth=2, markersize=12)
    plt.plot(5.0, 4.5, color='red', marker='x', linestyle='dashed', linewidth=2, markersize=12)
    plt.plot(8.0, 9.0, color='red', marker='o', linestyle='dashed', linewidth=2, markersize=12)
    plt.xlabel('X', fontsize=12)
    plt.ylabel('Y', fontsize=12)
    plt.savefig(dirName + 'test_last_trajectory.png')
    #plt.show()
    plt.close()
...
num_states = env.observation_space['location/1'].shape[0]
print("Size of State Space ->  {}".format(num_states))
num_actions = env.action_space['move/1'].shape[0]
print("Size of Action Space ->  {}".format(num_actions))

upper_bound = env.non_fluents['MAX_ACTION_BOUND/1'][0].astype(np.float64)
lower_bound = env.non_fluents['MIN_ACTION_BOUND/1'][0].astype(np.float64)
...
mov_var_vec = env.non_fluents['MOVE_VARIANCE_MULT/1']
...
tau = 0.005
max_pdf = scipy.stats.multivariate_normal.pdf([0, 0], [0, 0], [[mov_var_vec[0], 0.], [0., mov_var_vec[1]]])
